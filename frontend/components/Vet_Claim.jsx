...
export default function VetClaim() {
  ...
  const validateForm = () => {
    ...
    if (!insurancePattern.test(insuranceNumber)) {
      // Check if insuranceNumber has exactly 8 numbers
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        const response = await api.post('/vet_claim', formDataWithUserEmail)

        console.log(response.data)
        toast.success(
          'Your claim has been submitted to our veterinarian. You will be notified when you can chat.'
        )
      } catch (error) {
        ...
        <div className={styles.flexContainer}>
          <div className={styles.formContainer}>
            <form className={styles.form} onSubmit={handleSubmit}>
              ...
              <label className={styles.label} htmlFor="full-address">
                Full Address
              </label>
              <input
                type="text"
